...
def add_vn_calendar(df: pd.DataFrame, date_col: str = "Date") -> pd.DataFrame:
    ...
    days_to_bf, _ = _nearest_event_distance(pd.DatetimeIndex(d), bf_series)
    ...
    out["is_cyber_monday_window"] = ((days_to_bf <= -1) & (days_to_bf >= -3)).astype(int)
