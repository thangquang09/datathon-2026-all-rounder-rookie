...
from src.models.sales_forecasting import ARTIFACTS_DIR, DOCS_DIR, PACKAGE_ROOT, REPO_ROOT, SUBMISSION_PATH
...
def run_script(rel_path: str) -> None:
    script = REPO / rel_path if rel_path.startswith("src/") else ROOT / rel_path
    ...
    print(f"\n>>> {rel_path}")
    started = time.time()
    subprocess.run([sys.executable, str(script)], cwd=str(REPO), env=env, check=True)
...
def generate_calendar() -> Path:
    ...
    path = DOCS_DIR / "vietnam_calendar_events_deterministic_2012_2024.csv"
...
def write_reproduce_manifest(calendar_path: Path) -> None:
    manifest = {
        ...
        "pipeline": [
            ...
            "pipeline/infer_saved_direct_models.py",
...
def main() -> None:
    ...
    run_script("pipeline/infer_saved_direct_models.py")
    run_script("pipeline/build_direct_lgb_candidates.py")
    if not args.skip_visuals:
        run_script("src/visualization/sales_forecast_feature_importance.py")
