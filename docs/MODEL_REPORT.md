...
**Notebook:** `notebooks/model_reproduce_best_kaggle_solution.ipynb`
**Submission:** `data/processed/sales_forecast_submission/submission.csv`
...
**File:** `data/processed/sales_forecast_submission/submission.csv`
...
```
data/processed/sales_forecast_submission/submission.csv = 80% × M5_blend + 20% × Direct_LightGBM_regime_recovery
```

**Candidate file:** `data/processed/sales_forecast_submission/artifacts/final_candidates/submission_m5_lgb_direct_blend_80_20.csv`
...
Step 7: src/visualization/sales_forecast_feature_importance.py     → charts
```

Tất cả script nằm trong ``, chỉ đọc dữ liệu từ `../data/`.
...
| **Holidays** | `docs/sales_forecast_submission/vietnam_calendar_events_deterministic_2012_2024.csv` | Lễ Việt Nam, retail events sinh từ dương lịch |
...
| File | Nội dung |
|------|----------|
| `figures/top30_features_shap_overall.png` | Top 30 features theo SHAP (gộp Revenue + COGS) |
| `figures/top30_features_shap_by_target.png` | Top 30 features SHAP tách riêng Revenue vs COGS |
| `figures/top30_features_gain_overall.png` | Top 30 features theo LightGBM Gain |
| `figures/top30_feature_shortlist.csv` | Top 30 features + group + SHAP + Gain |
...
├── data/processed/sales_forecast_submission/submission.csv                          ← Final submission (80/20 blend)
├── README.md
├── notebooks/
...
│   ├── run_baselines.py
│   └── src/visualization/sales_forecast_pipeline_flowchart.py
├── docs/
...
│   └── src/visualization/sales_forecast_feature_importance.py
├── data/processed/sales_forecast_submission/artifacts/
│   ├── README.md
│   ├── cv_metrics.csv
│   ├── cv_weights.json
...
│   │   └── ... (other blend ratios)
│   └── ... (component CSVs, saved models, inference outputs)
└── components/
    ├── final_model.py
    ├── final_model_v2.py
    ├── final_model_v3.py
    └── final_model_v4.py
src/features/
└── operational_daily_features.py
src/utils/
└── calendar_vn.py
src/visualization/
└── sales_forecast_feature_importance.py
```
