# Pipeline Documentation — `src/models/sales_forecasting`
...
```
Final Submission (data/processed/sales_forecast_submission/submission.csv)
...
Built from `src/utils/calendar_vn.py` using deterministic rules from the Gregorian
...
**File**: `src/models/sales_forecasting/pipeline/forecast_pipeline.py`
...
**File**: `src/models/sales_forecasting/pipeline/explainable_forecast_factory.py`
...
**Files**: `src/models/sales_forecasting/components/final_model.py`, `src/models/sales_forecasting/components/final_model_v2.py`, `src/models/sales_forecasting/components/final_model_v3.py`, `src/models/sales_forecasting/components/final_model_v4.py`
...
**File**: `src/models/sales_forecasting/pipeline/build_direct_lgb_candidates.py`

The final `data/processed/sales_forecast_submission/submission.csv` is:
...
See `docs/sales_forecast_submission/tables/baseline_results.csv` and `docs/sales_forecast_submission/tables/pipeline_results.csv`.
...
├── data/processed/sales_forecast_submission/submission.csv                          ← Final Kaggle submission (548 rows)
...
│   └── src/visualization/sales_forecast_pipeline_flowchart.py               ← Regenerate docs/images/PIPELINE_FLOWCHART.png
│
├── notebooks/
...
│
├── ../../features/
│   └── operational_daily_features.py         ← V4 operational feature engineering module
│
├── ../../utils/
│   └── calendar_vn.py                        ← Deterministic Vietnamese calendar features
│
├── ../../visualization/
│   ├── sales_forecast_feature_importance.py  ← Feature importance visualization
│   └── sales_forecast_pipeline_flowchart.py  ← Report flowchart generation
│
├── pipeline/                            ← Pipeline A & B implementation
│   ├── __init__.py
...
│   └── src/visualization/sales_forecast_feature_importance.py           ← Feature importance visualization
│
└── data/processed/sales_forecast_submission/artifacts/                              ← Generated metrics, candidates, saved models, figures
    ├── README.md                           ← Artifact map and retention policy
    ├── final_candidates/                   ← Final-stage blend probes
    ├── saved_models/                       ← Saved direct LightGBM/Ridge models
    ├── inference/                          ← Saved-model inference outputs
    ├── figures/                            ← SHAP/gain plots
    └── *.csv / *.json / *.md               ← Metrics, audits, component candidates
```
...
jupyter notebook notebooks/model_reproduce_best_kaggle_solution.ipynb
...
# The final data/processed/sales_forecast_submission/submission.csv is produced by copying
